...
def sendhtml(data,subject=None,type=None,receiver='sunsw@blizzmi.com'):
    ...
    username = 'sunsw@blizzmi.com'
    password = 'Dum20397'

    sender = 'sunsw@blizzmi.com'
...
def expire():
    print(datetime.now().strftime('%Y-%m-%y %H:%M'))
    for issue in issues:
        ...
        if lend_days > 25:
            ...
            book_name = issue.fields.components[0].name
            book = book_name.replace('\t',' ')
            limit_day = 30 - lend_days
            if limit_day > 0 :
                # sendhtml('《%s》%s天到期，请知悉。' % (book,limit_day),subject='图书系统提示',receiver=sendto(name))
                print("-->%s's book expired soon" % name)
            else:
                sendhtml('《%s》超过一个月未归还，请尽快处理。' % book, subject='图书系统提示', receiver=sendto(name))
                print('-->逾期名单', name)
